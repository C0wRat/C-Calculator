#include <windows.h>
#include <stdio.h>
#include <stdlib.h>
#include "windowManager/windowManager.h"





#define DOT 31
#define CLEAR 32

typedef enum{
    none,
    addition,
    subtraction,
    multiplication,
    division,
}operation;

const char g_szClassName[] = "myWindowClass";
static char boxText[20];
static HWND box;
static HWND* buttons[17];
static char* input[20];
static double numIn = 0;
static double numOne = 0;
static double sum = 0;
static operation op = none;

// Step 4: the Window Procedure
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch(msg)
    {
        case WM_CLOSE:
            DestroyWindow(hwnd);
        break;
        case WM_DESTROY:
            PostQuitMessage(0);
        break;
        case WM_COMMAND:
            switch(wParam){
                case 0:
                    // const char* zero = "0";
                    if (numIn != 0){
                        strcat(input, "0");
                        SetWindowText(box, input);
                        numIn = atoi(input);
                    }else{
                        SetWindowText(box, "0");
                    }
                    break;
                case 1:
                    strcat(input, "1");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 2:
                    strcat(input, "2");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 3:
                    strcat(input, "3");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 4:
                    strcat(input, "4");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 5:
                    strcat(input, "5");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 6:
                    strcat(input, "6");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 7:
                    strcat(input, "7");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 8:
                    strcat(input, "8");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case 9:
                    strcat(input, "9");
                    numIn = atoi(input);
                    SetWindowText(box, input);
                    break;
                case BTN_DOT:
                    printf("Feature Not Added Yet!\n");
                    // GetWindowText(box, &boxText, 20);
                    //SetWindowText(box, "123");
                    break;
                
                case BTN_DIVIDE:
                    if(numOne == 0){
                        numOne = numIn;
                    }
                    op = division;
                    strcpy(input, ""); 
                    SetWindowText(box, input);
                    break;
                case BTN_MULTIPLY:
                    if(numOne == 0){
                        numOne = numIn;
                    }
                    op = multiplication;
                    strcpy(input, ""); 
                    SetWindowText(box, input);
                    break;
                case BTN_MINUS:
                    if(numOne == 0){
                        numOne = numIn;
                    }
                    op = subtraction;
                    strcpy(input, ""); 
                    SetWindowText(box, input);
                    break;
                case BTN_ADD:
                    if(numOne == 0){
                        numOne = numIn;
                    }
                    op = addition;
                    strcpy(input, ""); 
                    SetWindowText(box, input);
                    break;
                case BTN_Equals:
                    switch(op){
                        case none:
                            return 0;
                        case addition:
                            sum = numOne + numIn;
                            break;
                        case subtraction:
                            sum = numOne - numIn;
                            break;
                        case multiplication:
                            sum = numOne * numIn;
                            break;
                        case division:
                            if(numIn == 0){
                                SetWindowText(box, "ERROR");
                                return 0;
                            }
                            sum = numOne / numIn;
                            break;
                    }
                    numOne = sum;
                    char sumString[20];
                    // itoa(sum, sumString, 10);
                    gcvt(sum, 20, sumString);
                    SetWindowText(box, sumString);
                    break;
                case BTN_CLEAR:
                    printf("CLEARED\n");
                    numOne = 0;
                    numIn = 0;
                    sum = 0;
                    strcpy(input, ""); 
                    SetWindowText(box, "");
                    break;
                default:
                    break;
            }
        break;
        default:
            return DefWindowProc(hwnd, msg, wParam, lParam);
    }
    return 0;
}

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    LPSTR lpCmdLine, int nCmdShow)
{
    WNDCLASSEX wc;
    HWND hwnd;
    MSG Msg;

    //Step 1: Registering the Window Class
    wc.cbSize        = sizeof(WNDCLASSEX); // The size of the structure.
    wc.style         = 0; // Class Styles (CS_*), not to be confused with Window Styles (WS_*) This can usually be set to 0.
    wc.lpfnWndProc   = WndProc; // Pointer to the window procedure for this window class.
    wc.cbClsExtra    = 0; // Pointer to the window procedure for this window class.
    wc.cbWndExtra    = 0; // Amount of extra data allocated in memory per window of this type. Usually 0.
    wc.hInstance     = hInstance; // Handle to application instance (that we got in the first parameter of WinMain()).
    wc.hIcon         = LoadIcon(NULL, IDI_APPLICATION); // Large (usually 32x32) icon shown when the user presses Alt+Tab.
    wc.hCursor       = LoadCursor(NULL, IDC_ARROW);  // Cursor that will be displayed over our window.
    wc.hbrBackground = (HBRUSH)(COLOR_WINDOW+1); // Background Brush to set the color of our window.
    wc.lpszMenuName  = NULL; // Name of a menu resource to use for the windows with this class.
    wc.lpszClassName = g_szClassName; // Name to identify the class with.
    wc.hIconSm       = LoadIcon(NULL, BS_PUSHBUTTON); // Small (usually 16x16) icon to show in the taskbar and in the top left corner of the window.

    if(!RegisterClassEx(&wc))
    {
        MessageBox(NULL, "Window Registration Failed!", "Error!",
            MB_ICONEXCLAMATION | MB_OK);
        return 0;
    }

    // Step 2: Creating the Window
    hwnd = CreateWindowEx(
        WS_EX_CLIENTEDGE,
        g_szClassName,
        "Calculator",
        WS_SYSMENU,
        CW_USEDEFAULT, CW_USEDEFAULT, 260, 220,
        NULL, NULL, hInstance, NULL);

    if(hwnd == NULL)
    {
        MessageBox(NULL, "Window Creation Failed!", "Error!",
            MB_ICONEXCLAMATION | MB_OK);
        return 0;
    }


    box = NewWindow(W_TEXTBOX, hwnd, 10, 10, 230, 20, NULL, NULL); // Creating text box.
    buttonInit(buttons, hwnd); // Creating Buttons.

    ShowWindow(hwnd, nCmdShow);
    UpdateWindow(hwnd);

    // Step 3: The Message Loop
    while(GetMessage(&Msg, NULL, 0, 0) > 0)
    {
        TranslateMessage(&Msg);
        DispatchMessage(&Msg);
    }
    return Msg.wParam;
}